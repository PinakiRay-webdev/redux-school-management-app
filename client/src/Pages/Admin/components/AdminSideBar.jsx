...
const AdminSideBar = () => {
  ...
  return (
    <div
      ...
      } border-r border-zinc-300 fixed top-0 transition-all duration-150 ease-in-out`}
    >
      <p
        onClick={handleSideBar}
        className="bg-green-200 w-fit p-1 rounded-2xl absolute right-[-1rem] top-10 cursor-pointer"
      >
        {sideBarStatus ? <FaChevronLeft /> : <FaChevronRight />}
      </p>
          ...
          >
            ...
            >
              <NavLink
                ...
                to={`/adminDashboard/${Element.item}`}
              >
                <p>{<Element.icon />}</p>
              </NavLink>
            </p>
            <p
              className={`capitalize ${
                theme ? "text-[#ffffff]" : "text-[#051923]"
              } text-sm font-semibold ${sideBarStatus ? "block" : "hidden"} `}
            >
              <NavLink
                className={({ isActive }) =>
